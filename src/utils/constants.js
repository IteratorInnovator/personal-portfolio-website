...
export const projects = [
    {
        ...
        solution:
            "A marketplace where students post food requests and peers heading that way pick them up, with built-in chat, live tracking, and automatic payments to make transacting with strangers seamless.",
        outcome:
            "Buyers skip the queue and get food delivered between classes. Runners earn from trips they're already making. Campus eateries gain reach without building delivery infrastructure.",
        highlights: [
            "9 microservices split between atomic services (direct database access) and composite orchestrators (coordination only), enabling independent deployment and clear separation of concerns.",
            "Real-time WebSocket chat service in Go with AWS Keyspaces (Cassandra) for message persistence and Redis pub/sub for sub-100ms cross-pod broadcasting.",
            "Live GPS tracking service with WebSocket connections, Redis session state, and pub/sub broadcasting location updates every 3 seconds between buyers and runners.",
            "Event-driven order lifecycle using Apache Kafka (AWS MSK) with 4 event types triggering automatic chat room creation, location session management, payment transfers, and refunds.",
            "Google OAuth authentication via AWS Cognito with tokens stored in Expo SecureStore, plus Lambda post-confirmation triggers that auto-provision user documents in DynamoDB.",
            "Stripe Connect marketplace integration handling payment intent creation, fund holds during checkout, automatic transfers to runner accounts on delivery, and full refunds on cancellation.",
            "Terraform IaC provisioning 50+ AWS resources across EKS Fargate, API Gateway, Cognito, Lambda, DynamoDB, Keyspaces, ElastiCache Redis, MSK Kafka, S3, and Secrets Manager.",
            "GitLab CI/CD pipeline with SAST scanning, secret detection, Docker builds, and automated pushes to ECR with commit SHA and latest tags.",
            "API Gateway with Cognito JWT authorizer validating all requests before they reach the Application Load Balancer, plus internal service-to-service communication over private networking.",
            "Cross-platform React Native app built with Expo SDK 54, supporting iOS, Android, and web from a single TypeScript codebase with file-based routing via Expo Router.",
            "Composite orchestrator pattern where order-management coordinates checkout across user, payment, and order services, then publishes Kafka events consumed by delivery and payment management services.",
        ],
        metrics: [
            { label: "Microservices", value: "9" },
            { label: "AWS Services", value: "12+" },
            { label: "Chat Latency", value: "<100ms" },
        ],
        stack: [
            "React Native",
            "Expo",
